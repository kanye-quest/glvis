...
bool startVisualization(std::string input, std::string data_type) {
    ...
    vis_data data = readStream(input, data_type);
    ...
    if (data.field_type == 0 || data.field_type == 2)
    {
        ...
        if (data.mesh->SpaceDimension() == 2)
        {
            ...
            {
                vs = vss = new VisualizationSceneSolution(*data.mesh, data.sol, &data.normals);
            }
            else
            {
                vs = vss = new VisualizationSceneSolution(*data.mesh, data.sol);
            }
            if (data.grid_f)
            {
                vss->SetGridFunction(*data.grid_f);
            }
            if (data.field_type == 2)
            {
        ...
        else if (data.mesh->SpaceDimension() == 3)
        {
            VisualizationSceneSolution3d * vss;
            vs = vss = new VisualizationSceneSolution3d(*data.mesh, data.sol);
            if (data.grid_f)
            {
                vss->SetGridFunction(data.grid_f);
            }
        ...
        if (data.field_type == 2)
        {
            if (data.grid_f)
            {
                mesh_range = data.grid_f->Max() + 1.0;
            }
            else
            {
                mesh_range = data.sol.Max() + 1.0;
            }
        }
    }
    else if (data.field_type == 1)
    {
        if (data.mesh->SpaceDimension() == 2)
        {
            if (data.grid_f)
            {
                vs = new VisualizationSceneVector(*data.grid_f);
            }
            else
            {
                vs = new VisualizationSceneVector(*data.mesh, data.solu, data.solv);
            }
        }
        else if (data.mesh->SpaceDimension() == 3)
        {
            if (data.grid_f)
            {
                data.grid_f = ProjectVectorFEGridFunction(data.grid_f);
                vs = new VisualizationSceneVector3d(*data.grid_f);
            }
            else
            {
                vs = new VisualizationSceneVector3d(*data.mesh, data.solu, data.solv, data.solw);
            }
        }
    }
    ...
    if (vs)
    {
        ...
        if (data.mesh->SpaceDimension() == 2 && data.field_type == 2)
        {
            SetVisualizationScene(vs, 2, data.keys.c_str());
        }
        else
        {
            SetVisualizationScene(vs, 3, data.keys.c_str());
        }
    }
    return true;
}
